...
Return ONLY valid JSON with this exact shape:
{
  ...
  "platingGuides": ["string", "..."],
  "tableSetting": ["string", "..."],
  "serviceNotes": ["string", "..."],
  "ambianceGuide": ["string", "..."],
...
- Provide dayBeforePrep with 8-12 specific tasks tied to the menu.
- Provide platingGuides with 5 entries (one per course) referencing the dish by name.
- Provide tableSetting with 6-10 bullets customized to the menu and guest count.
- Provide serviceNotes with 6-10 bullets tailored to pacing and course order.
- Provide ambianceGuide with 6-10 bullets tied to the menu style and mood.
- Provide finalChecklist with 8-12 bullets focused on execution.
- Provide imagePrompts with 5 prompts (one per course) naming the dish.
- Avoid generic placeholders. Each bullet must reference a dish, ingredient, technique, or equipment.
- PlatingGuides must mention sauce placement, garnish, and temperature cues.
- TableSetting must mention specific glassware or cutlery tied to the courses.
...
- Steps must mention technique where applicable and remain practical for a skilled home cook.${compactGuidance}`;
    ...
    const normalizedExtras = {
      dayBeforePrep: normalizeStringList(details.dayBeforePrep),
      platingGuides: normalizeStringList(details.platingGuides),
      tableSetting: normalizeStringList(details.tableSetting),
      serviceNotes: normalizeStringList(details.serviceNotes),
      ambianceGuide: normalizeStringList(details.ambianceGuide),
      finalChecklist: normalizeStringList(details.finalChecklist),
      imagePrompts: normalizeStringList(details.imagePrompts),
    };
    res.json({
      ...details,
...
app.post("/api/generate-cookbook", async (req, res) => {
  ...
  global.cookbooks[cookbookId] = {
    ...
    dayBeforePrep: dayBeforePrep || null,
    platingGuides: platingGuides || null,
    tableSetting: tableSetting || null,
    serviceNotes: serviceNotes || null,
    ambianceGuide: ambianceGuide || null,
    finalChecklist: finalChecklist || null,
